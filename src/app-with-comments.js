...
//General tips / improvements
// - Add loading state + show a loader when data is fetching
// - Functions like formatForecast should be defined out of component or can even be in a different file (readability & reusability)
// - create a sepetate component for buttons, lists, ... (readability & reusability & easier to maintain on bigger projects)
//    - This way you could also split your logic and styling (css)
// - for naming classes we generally user BEM standard in our projects https://sparkbox.com/foundry/bem_by_example

function App() {
    const [cityName, setCityName] = useState("Antwerp");
    const [fetchErrorCity, setFetchErrorCity] = useState(false);
    const [weather, setWeather] = useState({
        temperature: "", //An empty string as default value is not a good practice, you can use null here. especially if the final data is a number
        feelsLike: "",
        conditionText: "",
        locationText: "",
        localtime: "",
    ...
    const fetchData = async (cityName) => {
        ...
        const options = {
            ...
            params: {q: cityName},
            headers: {
                'X-RapidAPI-Key': '734bc29f81msh85bd8e1dea8c988p1bb9c2jsn3133e9a810a3', //Try to put your key in an .env variable 
        ...
        try {
            ...
            setWeather({
                cityName,
                temperature: `${tempC}°C`, //It's not ideal to store °C in the string. This can be added in the HTML. (for example more difficult to maintain if user can switch between celius and fahrenheit)
                feelsLike: `${feelsLikeC}°C`, //same
                humidity: `${humidity}%`, // same for %
                wind_kph: `${wind_kph} km/h`, // same for km/h
                conditionText,
                icon,
                name,
                country,
    ...
    return (
        <Fragment> {/*You can use <></> instead of <Fragment></Fragment>, same output but no need for extra import*/}
